"""
EOS Meeting Report — 2-Page PDF from up to 6 Analysis Excel Files
=================================================================
Reads the output workbooks produced by analyze.py (the 5-sheet Excel
files) and consolidates them into a tight, 2-page PDF designed for
a Level 10 / EOS-style meeting.
...
Usage:
  python eos_report.py file1.xlsx file2.xlsx ...          # up to 6 files
  python eos_report.py file1.xlsx file2.xlsx -o report.pdf
"""
...
# PDF Builder
# ---------------------------------------------------------------------------
class EOSReport(FPDF):
    """Two-page PDF formatted for an EOS Level 10 meeting."""
...
# Main entry point
# ---------------------------------------------------------------------------
def generate_eos_report(excel_paths, output_path=None):
    """Generate a 2-page EOS meeting PDF from up to 6 analysis Excel files.
    ...
    if output_path is None:
        ...
        output_path = f"EOS_Report_{timestamp}.pdf"

    pdf.output(output_path)
    print(f"\n  EOS Report: {output_path}")
    return output_path


def generate_eos_report_bytes(excel_paths):
    """Generate EOS PDF and return as bytes (for Streamlit download).
...
def main():
    ...
    if not excel_paths:
        print("Usage: python eos_report.py file1.xlsx [file2.xlsx ...] [-o output.pdf]")
        print("  Reads up to 6 analysis Excel files and generates a 2-page EOS meeting PDF.")
        sys.exit(1)

    generate_eos_report(excel_paths, output_path)
