...
class TestSingleSheetFallback:
    ...
    def test_load_oee_data_single_data_sheet_with_header_offset(self, tmp_path):
        path = tmp_path / "single_data_offset.xlsx"
        base = pd.DataFrame(
            [
                {
                    "Date": "2026-02-18",
        ...
        # Write two junk rows before the real header row.
        with pd.ExcelWriter(path, engine="xlsxwriter") as writer:
            pd.DataFrame([["Export generated by MES"], ["Do not edit this row"]]).to_excel(
                writer, sheet_name="Data", header=False, index=False
            )
            base.to_excel(writer, sheet_name="Data", startrow=2, index=False)
    ...
    def test_load_oee_data_single_sheet_loose_headers(self, tmp_path):
        path = tmp_path / "single_data_loose.xlsx"
        df = pd.DataFrame(
            [
                {
