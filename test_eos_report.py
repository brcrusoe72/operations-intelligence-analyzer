"""
Tests for EOS Meeting Report generation.

Run: python -m pytest test_eos_report.py -v
"""
...
class TestEOSReportPDF:
    ...
    def test_pdf_is_valid(self, mock_data, tmp_path):
        ...
        pdf.build_page1(mock_data)
        pdf.build_page2(mock_data)
        output_path = str(tmp_path / "test_eos.pdf")
...
class TestGenerateEOSReport:
    def test_end_to_end(self, tmp_path):
        ...
        output = str(tmp_path / "eos_output.pdf")
        result = generate_eos_report(paths, output)
    ...
    def test_single_file(self, tmp_path):
        ...
        output = str(tmp_path / "eos_single.pdf")
        result = generate_eos_report([p], output)
    ...
    def test_max_six_files(self, tmp_path):
        ...
        output = str(tmp_path / "eos_max.pdf")
        result = generate_eos_report(paths, output)
        assert os.path.exists(result)
    ...
    def test_missing_file_skipped(self, tmp_path):
        ...
        output = str(tmp_path / "eos_missing.pdf")
        result = generate_eos_report([p, "/nonexistent/file.xlsx"], output)
        assert os.path.exists(result)

    def test_no_valid_files_raises(self):
        with pytest.raises(ValueError, match="No valid"):
            generate_eos_report(["/nonexistent/a.xlsx", "/nonexistent/b.xlsx"])
